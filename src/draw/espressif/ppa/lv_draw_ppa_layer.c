...
bool lv_draw_ppa_layer_recolor_opa_supported(const lv_draw_image_dsc_t * dsc)
{
    ...
    if(dsc->skew_x != 0 || dsc->skew_y != 0) return false;
    ...
    if(src_cf != LV_COLOR_FORMAT_RGB565 && src_cf != LV_COLOR_FORMAT_RGB888
       && src_cf != LV_COLOR_FORMAT_XRGB8888) return false;

    if(!ppa_dest_cf_supported(dsc->base.layer->color_format)) return false;
    return true;
}
                                                       ...
                                                       const lv_area_t * coords)
{
    ...
    if(src_bpp == 0 || dest_bpp == 0) return;
    ...
    if(keep_src_ratio <= 0.0f) keep_src_ratio = 1.0f / 256.0f;
    if(keep_src_ratio >= 1.0f) keep_src_ratio = 255.0f / 256.0f;
    {
