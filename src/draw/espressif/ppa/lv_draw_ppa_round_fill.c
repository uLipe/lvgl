                                                  ...
                                                  const lv_area_t * coords)
{
    ...
    if(r > short_side / 2) r = short_side / 2;
    if(r <= 0) return;
    ...
    for(int32_t i = 0; i < r; i++) {
        ...
        if(inner_offset < 0) inner_offset = 0;
        if(inner_offset > r) inner_offset = r;
                                                ...
                                                const lv_area_t * buf_area, uint32_t color)
{
    ...
    if(ret != ESP_OK) {
        ...
        LV_LOG_ERROR("PPA round fill scanline failed: %d", ret);
    }
}
