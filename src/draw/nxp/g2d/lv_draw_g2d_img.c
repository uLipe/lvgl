...
void lv_draw_g2d_img(lv_draw_g2d_unit_t * draw_g2d_unit, const lv_draw_image_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    lv_area_t clip_area;
    lv_area_copy(&clip_area, draw_g2d_unit->base_unit.clip_area);
    ...
    struct g2d_surface dst_surf = _g2d_set_dst_surf(dst_buf, &blend_area, dest_stride, dest_cf, dsc);

    _g2d_blit(draw_g2d_unit->g2d_handle, dst_buf, dst_surf, src_buf, src_surf);
}
...
{
    struct g2d_buf * src_buf = lv_search_buf_map((void *)img_dsc->data);

    if(src_buf == NULL) {
        src_buf = g2d_alloc(img_dsc->data_size, 1);
        memcpy((int *)src_buf->buf_vaddr, img_dsc->data, img_dsc->data_size);
        lv_insert_buf_map((void *)img_dsc->data, src_buf);
    }
